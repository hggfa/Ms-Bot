...
client.user.setGame(`-help|by omar salem`,"https://www.twitch.tv/Music-bot")
  console.log('')
  console.log('')
  console.log('╔[═════════════════════════════════════════════════════════════════]╗')
...
${videos.map(video2 => `[**${++index}**] **${video2.title}**`).join('\n')}`)
          .setFooter(`${msg.guild.name}`)
            .setThumbnail('https://images-ext-2.discordapp.net/external/y3zU9xD-YouGn2nG1-Zr9V6xkZw19PnKX8gUJIfZQoA/https/cdn.discordapp.com/attachments/528703259917942800/530129817140723732/preferences-system-icon.png')
...
function play(guild, song) {
  ...
  .setTitle(`**${ idk.title}**`)
  .setURL( idk.url)
  .addField('وقت مقطع :' , `${song.time}`, true)
  .addField('اسم القناة :' , `${song.best}`, true)
  .addField('تاريخ نشر المقطع :' , `${ idk.datePublished}`, true)
  .addField('المشاهدات  :' , `${ idk.views}`, true)
  .addField('اللايك👍 :' , `${ idk.likeCount}`, true)
  .addField('الديسلايك👎 :' , `${ idk.dislikeCount}`, true)
  .setImage(`${song.eyad}`)
  .setThumbnail('https://images-ext-2.discordapp.net/external/y3zU9xD-YouGn2nG1-Zr9V6xkZw19PnKX8gUJIfZQoA/https/cdn.discordapp.com/attachments/528703259917942800/530129817140723732/preferences-system-icon.png')
  .setColor('#ff0000')
  .setTimestamp()
           ...
           var mentionned = message.mentions.members.first();
             ...
             if(ment) {
               ...
               getvalueof = message.author;
                                           ...
                                           let url = getvalueof.dis
					   
					   
					  AvatarURL.endsWith(".webp") ? getvalueof.displayAvatarURL.slice(5, -20) + ".png" : getvalueof.displayAvatarURL;
    ...
    message.channel.send({
        embed: new Discord.RichEmbed()
            ...
            .addField('``My ID``' , `[ ${client.user.id} ]` , true)
			      ...
			      .setFooter('By | omar salem - Mál ,קΣsĦǾǾ🔥 ❥ {G}#6201')
    })
}
});
...
client.on('guildCreate', guild => {
  var embed = new Discord.RichEmbed()
  .setColor(0x5500ff)
  .setDescription(`**Thank You For Adding The Bot To Your Server If You Need Any Help In The Bot Go To Suuport Server** https://discord.gg/N5PmEfu`)
...
client.on('message', message => {
        ...
        .setAuthor(` ${message.author.username} `, message.author.avatarURL)      
        .setTitle(`➡ Click Here `)
        .setURL(`https://discordapp.com/api/oauth2/authorize?client_id=526491864740921365&permissions=8&scope=bot`)
...
client.on('guildMemberAdd', member=> {
    member.addRole(member.guild.roles.find("name","Pro"));
    });
